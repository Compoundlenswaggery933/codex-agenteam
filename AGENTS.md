...
AgenTeam (`codex-agenteam`) is a Codex plugin that provides role-based team collaboration for AI-assisted development. Roles (architect, implementer, reviewer, test-writer) are defined as YAML templates, materialized into Codex-native custom agents (`.codex/agents/*.toml`), and orchestrated through a configurable pipeline.
