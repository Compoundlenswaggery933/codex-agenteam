...
def _pass_required_fields(config: dict, result: ValidationResult) -> None:
    if "version" not in config:
        result.diagnostics.append(Diagnostic(
            Severity.ERROR, "version", "Missing required field: version", "E001",
        ))
        return
    ...
    if version not in VALID_VERSIONS:
        result.diagnostics.append(Diagnostic(
            ...
            f"Invalid version: '{version}'. Must be one of: {', '.join(sorted(VALID_VERSIONS))}",
            "E001",
        ))
...
def _pass_top_level_enums(config: dict, result: ValidationResult) -> None:
    ...
    if fvmr is not None:
        if not isinstance(fvmr, int) or isinstance(fvmr, bool) or fvmr < 0:
            result.diagnostics.append(Diagnostic(
                Severity.ERROR, "final_verify_max_retries",
...
def _pass_legacy_keys(config: dict, result: ValidationResult) -> None:
    ...
    if not isinstance(team, dict):
        result.diagnostics.append(Diagnostic(
            Severity.ERROR, "team", "'team' must be a mapping", "E017",
        ))
        return
    ...
    if legacy_pipeline is not None:
        if legacy_pipeline not in VALID_PIPELINES:
            ...
            result.diagnostics.append(Diagnostic(
                Severity.WARNING, "team.pipeline",
    ...
    if isinstance(pw, dict):
        ...
        if mode is not None:
            if mode not in VALID_WRITE_MODES:
                result.diagnostics.append(Diagnostic(
                    Severity.ERROR, "team.parallel_writes.mode",
...
def _pass_pipeline_stages(config: dict, result: ValidationResult) -> None:
    ...
    if not isinstance(stages, list):
        result.diagnostics.append(Diagnostic(
            Severity.ERROR, "pipeline.stages",
            "pipeline.stages must be a list", "E004",
        ))
        return
    ...
    for i, stage in enumerate(stages):
        ...
        if not isinstance(stage, dict):
            result.diagnostics.append(Diagnostic(
                ...
                f"{prefix}: stage must be a mapping", "E005",
            ))
            continue
        ...
        if not name or not isinstance(name, str):
            result.diagnostics.append(Diagnostic(
                Severity.ERROR, f"{prefix}.name",
                f"{prefix}.name: stage must have a 'name' string field", "E005",
            ))
            continue
        ...
        if roles is not None and (not isinstance(roles, list)):
            result.diagnostics.append(Diagnostic(
                Severity.ERROR, f"{prefix}.roles",
                f"{prefix}.roles: must be a list", "E006",
            ))

        gate = stage.get("gate")
        if gate is not None and gate not in VALID_GATE_TYPES:
            result.diagnostics.append(Diagnostic(
                Severity.ERROR, f"{prefix}.gate",
        ...
        if max_retries is not None:
            if not isinstance(max_retries, int) or isinstance(max_retries, bool) or max_retries < 0:
                result.diagnostics.append(Diagnostic(
                    Severity.ERROR, f"{prefix}.max_retries",
                    f"{prefix}.max_retries: must be a non-negative integer, got '{max_retries}'",
                    "E016",
                ))
    ...
    if isinstance(fv, str):
        ...
        for i, item in enumerate(fv):
            if not isinstance(item, str):
                result.diagnostics.append(Diagnostic(
                    Severity.ERROR, f"final_verify[{i}]",
                    f"final_verify[{i}]: must be a string, got {type(item).__name__}",
                    "E013",
                ))
    else:
        result.diagnostics.append(Diagnostic(
            Severity.ERROR, "final_verify",
...
def _pass_cross_references(
    config: dict, resolved_roles: dict, result: ValidationResult,
) -> None:
        ...
        for role_name, role_cfg in role_overrides.items():
            if isinstance(role_cfg, dict):
                ...
                if re_val is not None and re_val not in VALID_REASONING_EFFORT:
                    result.diagnostics.append(Diagnostic(
                        Severity.ERROR, f"roles.{role_name}.reasoning_effort",
...
def _pass_profiles(config: dict, result: ValidationResult) -> None:
    ...
    for profile_name, profile_def in profiles.items():
        ...
        if not isinstance(profile_def, dict):
            result.diagnostics.append(Diagnostic(
                Severity.ERROR, prefix,
                f"{prefix}: profile must be a mapping", "E010",
            ))
            continue
        ...
        if not stages or not isinstance(stages, list):
            result.diagnostics.append(Diagnostic(
                Severity.ERROR, f"{prefix}.stages",
                f"{prefix}.stages: must be a non-empty list", "E010",
            ))
            continue
